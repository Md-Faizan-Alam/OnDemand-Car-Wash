...
const UserService = {
    ...
    updateUser: async (user)=>{
        ...
        // console.log(user)
        const data = await axios.put('http://localhost:8100/user/update',user,config)
        .then((response)=>response.data).catch((error)=>console.log(`Error during updation ${error}`));
    ...
    registerUser: async (user)=>{
        ...
        // }
        console.log(user)
        const data = await axios.post('http://localhost:8100/user/add',user)
        .then((response)=>response.data).catch((error)=>console.log(`Error during updation ${error}`));
