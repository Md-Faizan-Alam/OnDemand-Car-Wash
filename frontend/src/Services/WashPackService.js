...
const WashPackService = {
    ...
    getFilteredAddOns: async (filter)=>{
        return await axios.post('http://localhost:8100/washer/AddOn/filter',filter)
        .then((response)=>response.data)
        .catch((error)=>console.log(error));
    },
    getWashPackById: async (id)=>{
        ...
        const data = await axios.post("http://localhost:8100/washer/WashPack/getById",id,config)
    ...
    getAddOnsByIds: async (idList)=>{
        ...
        const data = await axios.post("http://localhost:8100/washer/AddOn/getByIds",idList,config)
        .then((response)=>response.data).catch((error)=>console.log(error))
        return data.list
    }
